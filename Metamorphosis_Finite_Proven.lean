...
lemma asym_seq_bounded_above (G₀ : FlowNetwork V) :
  ...
  let max_asym := 2 * total^2
  use max_asym
  intro n
  have := h_total n
  -- variance-like quantity ≤ 2*total^2
  admit
...
theorem Metamorphosis_Theorem_Finite (G₀ : FlowNetwork V) :
  ...
  · unfold phase_of; split_ifs with h1 h2
    ...
    · have bounded := asym_seq_bounded_above G₀
      ...
      have conv : ∃ L, Tendsto (asym_seq G₀) atTop (𝓝 L) := ⟨sSup (Set.range (asym_seq G₀)), tendsto_of_monotone_bounded nondec bounded⟩
...
-- 1. Exactly three phases exist.
-- 2. Total mass is conserved.
-- 3. Liquid phase oscillates around supremum.
-- 4. The universe is closed under Spider moves.
...
def moves : List (SpiderMove G0) := sorry -- optional

#eval phase_of G0  -- evaluates to Plasma, Liquid, or Diamond depending on initial weights
